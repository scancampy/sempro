...
class Ijinlab extends CI_Controller {
    ...
    public function index() {
		...
		foreach($roles as $role) {
			if($role->roles == 'student') {
				$data['roles'] = 'student';
				$data['ijinlab'] = $this->Ijin_lab_model->get($info[0]->nrp);
			} else if($role->roles == 'lecturer') {
				$data['roles'] = 'lecturer';
				$data['ijinlab'] = $this->Ijin_lab_model->get_by_npk($roles[0]->username);	
				

				if($this->input->post('btnvalidasidosbing')) {
					$this->Ijin_lab_model->dosbing_validate($this->input->post('hid_ijin_id'),$roles[0]->username);
					$this->session->set_flashdata('notif', 'success');
					$this->session->set_flashdata('message', 'Sukses memvalidasi ijin lab');
					redirect('ijinlab');
				}	
			} else if($role->roles == 'kalab') {
				$data['is_kalab'] = $info[0]->lab_id;	
				$data['ijinlab'] = $this->Ijin_lab_model->get_by_npk_kalab($roles[0]->username, $info[0]->lab_id);	
				if($this->input->post('btnvalidasikalab')) {
					$this->Ijin_lab_model->kalab_validate($this->input->post('hid_ijin_id'),$roles[0]->username);
					$this->session->set_flashdata('notif', 'success');
					$this->session->set_flashdata('message', 'Sukses memvalidasi ijin lab');
					redirect('ijinlab');
				}	
				...
				$data['roles'] = 'wd';
				$data['is_wd'] = true;
				$data['ijinlab'] = $this->Ijin_lab_model->get_where("ijin_lab.wd_validated_date IS NULL AND ijin_lab.is_deleted = 0");	

				if($this->input->post('btnvalidasiwd')) {
					$this->Ijin_lab_model->wd_validate($this->input->post('hid_ijin_id'),$roles[0]->username);
					$this->session->set_flashdata('notif', 'success');
					$this->session->set_flashdata('message', 'Sukses memvalidasi ijin lab');
					redirect('ijinlab');
				}	
