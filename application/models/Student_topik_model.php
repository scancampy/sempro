...
class Student_topik_model extends CI_Model {
        ...
        public function get($nrp='',$id='', $is_deleted = 0, $is_rejected =0, $topik_id = '', $kalab_npk_verified = null) {
                if($nrp != '') {
                ...
                $this->db->select('student_topik.*, student.nama as "studentname", topik.nama, lab.nama as "namalab", l1.nama as "guardianname", l2.nama as "kalabnama", l3.nama as "wdnama", l4.nama as "wdnamareject", dosbing1.nama as "dosbing1nama", dosbing2.nama as "dosbing2nama", pembuat.nama as "pembuat"');
        ...
        public function get_wd_topic() {
                ...
                $this->db->join('student', 'student.nrp = student_topik.student_nrp', 'left');
                $this->db->select('student_topik.*, topik.nama, student.nama as "namamhs"');
               $p = $this->db->get_where('student_topik', array('student_topik.guardian_npk_verified != ' => null, 'student_topik.kalab_npk_verified !=' => null, 'student_topik.is_verified' => 0, 'student_topik.is_deleted' => 0));
               return $p->result();
        }
        ...
        public function get_kalab_topic($npk) {
                ...
                if($q->num_rows() > 0) {
                        ...
                        foreach($p->result() as $value) {
                                ...
                                $this->db->join('student', 'student.nrp = student_topik.student_nrp', 'left');
                                $this->db->select('student_topik.*, topik.nama, student.nama as "namamhs"');
                                $r = $this->db->get_where('student_topik', array('topik_id' => $value->id,'student_topik.guardian_npk_verified !=' => null, 'student_topik.kalab_npk_verified' => null));
        ...
        public function get_kalab_topic_need_dosbing($npk) {
                ...
                if($q->num_rows() > 0) {
                        ...
                        foreach($p->result() as $value) {
                                ...
                                $this->db->join('student', 'student.nrp = student_topik.student_nrp', 'left');
                                $this->db->select('student_topik.*, topik.nama, student.nama as "namamhs"');
                                $r = $this->db->get_where('student_topik', array('topik_id' => $value->id,'is_verified' => 1, 'lecturer1_npk' => null, 'lecturer2_npk' => null));
        ...
        public function update_judul($id, $judul) {
                $data = array('judul_created' => date('Y-m-d H:i:s'), 'judul' => $judul);
                $this->db->where('id', $id);
                $this->db->update('student_topik', $data);
        }

        public function update_dosbing($id, $lecturer1_npk,$lecturer2_npk) {
                $data = array('lecturer_created' => date('Y-m-d H:i:s'), 'lecturer1_npk' => $lecturer1_npk, 'lecturer2_npk' => $lecturer2_npk);
                $this->db->where('id', $id);
                $this->db->update('student_topik', $data);       
        }
